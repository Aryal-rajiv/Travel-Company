...
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=AW-11112766176"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'AW-11112766176');
</script>
		...
		if(is_single() && !empty($pp_page_bg) && !grandtour_is_woocommerce_page())
		{
		    ...
		    switch($post_type)
		    {
		    	case 'tour':
					...
					{
						//Check if your overwrite menu transparent option
						$page_menu_normal = get_post_meta($current_page_id, 'page_menu_normal', true);
						
						if(empty($page_menu_normal)) {
